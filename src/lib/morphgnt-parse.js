...
export function parseGreekMorph(morph) {
  var result = "",
    gender,
    morphArray,
    number,
  ...
  if (partOfSpeech === "V") {
    ...
    if (Case !== undefined) {
      if (morphologyDictionary.greek.Case[Case] !== undefined) {
        ...
        if (parseInt(Case[0], 10) > 0) {
          // second future, second aorist and second perfect
          tense = Case[0] + Case[1];
          voice = Case[2];
          mood = Case[3];
        } else {
    ...
    if (morphArray[1] !== undefined) {
      if (morphologyDictionary.greek.Case[Case] !== undefined) {
        //there are some nouns that have a 3 letter case code
        result += morphologyDictionary.greek.Case[Case];
      } else {
