...
import harmonised from "../data/harmonised.js";
...
export function getHarmonisedReference({ book, chapter, verseNumber, index }) {
  ...
  return {
    ...
    verseNumber: harmonisedReference[1] ? harmonisedReference[1] - 1 : null,
    index,
  };
}
...
export const getSyncReference = (stateReference) => {
  const syncedReference = stateReference.map((reference) => {
    ...
    if (stateReference[0].book === "Harmony") {
      const harmonisedReference = getReferenceFromHarmony(stateReference[0]);
      newSyncedReference = {
        ...newSyncedReference,
        ...harmonisedReference,
...
function getReferenceFromHarmony({ chapter, verse, version }) {
  ...
  const harmonisedReference = harmonisedVerses
    .filter((singleharmonisedReference) => singleharmonisedReference.length > 0)
...
export const getGroupedResults = (
  ...
  if (selectedGroup === "book") {
    ...
    resultsToDisplay = groupBy(resultsArray, function (item) {
      if (Array.isArray(item)) {
        return (
          bible.getTranslatedBookName(item[0], interfaceLanguage) +
          " " +
          item[1]
        );
      }
...
export function getHashFromAction(action) {
